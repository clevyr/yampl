...
func Test_templateReader(t *testing.T) {
	...
	tests := []struct {
		...
		{"dynamic (deprecated)", args{config.New(), strings.NewReader("a: a #yampl {{ .Value }}")}, "a: a #yampl {{ .Value }}\n", require.NoError},
		{"dynamic", args{config.New(), strings.NewReader("a: a #yampl {{ current }}")}, "a: a #yampl {{ current }}\n", require.NoError},
